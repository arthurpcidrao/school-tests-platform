...
export default function EditQuestionPage() {
  ...
  useEffect(() => {
    ...
    const fetchQuestion = async () => {
      try {
        const res = await fetch(`${API_BASE_URL}/api/exams/questions/${params.id}`, {
          headers: { "Authorization": `Bearer ${token}` }
        });
        if (res.ok) {
          const data = await res.json();
          setQSubject(data.subject);
          setQStem(data.stem);
  ...
  const handleSaveQuestion = async () => {
    ...
    try {
      const resQ = await fetch(`${API_BASE_URL}/api/exams/questions/${params.id}`, {
        method: "PUT",
        headers: {
        ...
        <div className="w-full max-w-[896px] space-y-md pb-xl">
          ...
          <div className="flex items-center justify-between mb-6">
            <div className="flex items-center gap-4">
              ...
              <div>
                <h1 className="font-h1 text-h1">Editar Questão</h1>
                <p className="text-text-secondary">Atualize os dados desta questão no banco global.</p>
              </div>
            </div>
          </div>
